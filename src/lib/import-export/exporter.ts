 ...
 *
 * ZIP 文件包含：
 * - clips.json：所有剪贴板数据的 JSON 格式
 * - clips.csv：文本和链接类型剪贴板的 CSV 格式
 *
 * @param clips - 要导出的剪贴板条目数组
 * @param filename - 可选的自定义文件名（不含扩展名），默认为 "clipsync-export"
 */
export async function exportAsZIP(clips: Clip[], filename?: string): Promise<void> {
  const zip = new JSZip();
  ...
  zip.file("clips.json", JSON.stringify(exportData, null, 2));
  ...
  const bom = "\uFEFF";
  zip.file("clips.csv", bom + [header, ...rows].join("\n"));
