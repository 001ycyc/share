...
export async function createClip(
  ...
  if (input.file) {
    ...
    // 生成唯一文件路径：clips/{userId}/{timestamp}_{filename}
    const ext = input.file.name.split(".").pop() || "bin";
    const timestamp = Date.now();
    const safeName = input.file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
    const filePath = `clips/${userId}/${timestamp}_${safeName}`;

    // 将 File 转换为 ArrayBuffer 再转为 Uint8Array
 ...
 * 恢复已软删除的剪贴板记录。
 */
export async function restoreClip(clipId: string, userId: string): Promise<void> {
  const supabase = createClient();
  const { error } = await supabase
    .from("clips")
  ...
  if (error) {
    throw new AppError("CLIP_005", {
      message: `Failed to restore clip: ${error.message}`,
      details: { clipId, userId, error },
    });
  }
}
 ...
 * 获取已软删除的剪贴板记录列表。
 */
export async function getDeletedClips(
  userId: string,
  page = 1,
  pageSize = 20
): Promise<ClipListResponse> {
  ...
  if (error) {
    throw new AppError("CLIP_005", {
      message: `Failed to fetch deleted clips: ${error.message}`,
      details: { userId, error },
    });
  }
  ...
  const clips = (data as Clip[]) ?? [];
  const total = count ?? 0;
  return { clips, total, page, pageSize, totalPages: Math.ceil(total / pageSize) };
}

/**
 * 永久删除剪贴板记录（物理删除）。
 */
export async function permanentDeleteClip(clipId: string, userId: string): Promise<void> {
  const supabase = createClient();
  const { error } = await supabase
    .from("clips")
    .delete()
  ...
  if (error) {
    throw new AppError("CLIP_005", {
      message: `Failed to permanently delete clip: ${error.message}`,
      details: { clipId, userId, error },
    });
  }
}
...
/**
 * 自动清理回收站中超过指定天数的记录（物理删除）。
 *
 * @param userId - 当前登录用户 ID
 * @param retentionDays - 保留天数，默认 7 天
 * @returns 被删除的记录数量
 */
export async function purgeExpiredDeletedClips(
  userId: string,
  retentionDays = 7
): Promise<number> {
  const supabase = createClient();
  ...
  if (error) {
    console.error("[purgeExpiredDeletedClips] Failed:", error.message);
    return 0;
  }
