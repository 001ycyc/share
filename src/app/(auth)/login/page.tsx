  ...
  return (
    <Card>
      <CardHeader className="text-center">
        ...
        <CardTitle className="text-2xl font-bold">登录 ClipSync</CardTitle>
        <CardDescription>
          输入您的账号信息以登录到 ClipSync
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        {isRegistered && (
          <div className="rounded-lg bg-green-50 px-4 py-3 text-sm text-green-600 border border-green-200">
            注册成功！请查看邮箱确认链接后再登录。如果未开启邮箱确认，可直接登录。
          </div>
        )}
...
export default function LoginPage() {
  return (
    <Suspense fallback={<Card><CardContent className="p-8 text-center text-muted-foreground">加载中...</CardContent></Card>}>
      <LoginForm />
    </Suspense>
  );
}
